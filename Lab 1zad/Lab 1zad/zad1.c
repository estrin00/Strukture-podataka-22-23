...
{
	student* st = NULL;
	int bodoviSt=0, brojac=0;
	float relativniBodovi[MAX_SIZE];
	char dat[MAX_SIZE] = "popis.txt";


	brojac=procitajBrSt(dat);
	printf("Broj studenata u datoteci: %d\n",brojac);

	st=alokacijaSt(dat, brojac);
	...
	printf("\nRELATIVNI BODOVI (max broj bodova je 25):\n");
	for (int i = 0; i < brojac; i++)
	{
		relativniBodovi[i] = (float)st[i].bodovi / 25 * 100;
